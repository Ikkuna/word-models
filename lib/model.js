...
const svd = require('../node_modules/SVD/svd.js').svd


// LOAD MODEL //

module.exports = function loadModel( file, callback, shouldnorm ) {
	var model = {};
	var vocab = [];
	var words;
	var size;
	...
	model.similarity = function similarity( word1, word2 ) {
		...
		for ( i = 0; i < words; i++ ) {
			if ( vocab[i].word === word1 || vocab[i].word === word2 ) {
				vecs.push(normalize(vocab[i].values));
			}
		}
		if ( vecs.length === 2 ) {
			sum = 0;
	...
	model.getNearestWord = function getNearestWord( vec ) {
		...
		if ( vec instanceof WordVec === true ) {
			newvec = normalize(vec.values);
		} else newvec = normalize( vec );

		for ( c = 0; c < words; c++) {
			var dist = 0;
			const normalized = normalize(vocab[c].values)
			for ( a = 0; a < size; a++ ) {
				dist += vec[a] * normalized[a];
			}
			if ( c === 0 || dist > bestd ) {
				bestd = dist;
				bestw = vocab[c].word;
	...
	function readTxt( file ) {
		...
		rl.on( 'line', function( line ) {
			...
			if ( counter === 0 ) {
				...
				if ( word == 'AA') console.log('VALUES',word, values)
				...
				// ---------------------------------------
				// vocab.push( o );
				vocab.push( new WordVec( word, values ) )
				counter++;
			}
		});
